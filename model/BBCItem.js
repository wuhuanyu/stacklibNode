...
BBC.tags = ['world','africa','australia','latin','middle_east','asia','uk','business','tech','entertainment_arts','health'];
BBC.fields = ['source','crawled_at','timestamp','summary','title','url','text','image_urls','tag','id'];

BBC.findByTag = function (tag,num,crawl,fields) {
    return BBC.find({tag: tag})
        .where('crawled_at').lt(crawl)
        .sort({crawled_at: -1})
        .limit(num)
        .select(fields.join(' '));
};
...
BBC.findByText = (text,fields,count,crawled_at)=>{
    return BBC.find({
        ...
        .select(fields)
        .where('crawled_at').lt(crawled_at)
        .limit(count)
        .sort({'crawled_at':-1});
};
